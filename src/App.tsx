...
import { Trophy, Filter, RefreshCw, Search, AlertCircle, TrendingUp, Ticket, Radio, Menu, X, Zap, Flame, Shield, Activity, Crown, Star, Sun, Compass, Award, BarChart3, Home, BookOpen, ShieldOff, AlertTriangle } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { syncQuotaFromAPI, getQuotaUsage } from './services/apiQuotaService';
...
import { fetchRealScouting, buscarEstatisticasMedias, buscarH2H } from './services/scoutingService';
import BancaModal from './components/BancaModal';
import { getBancaAtual, setBancaAtual } from './services/bancaService';
import { registrarPrevisao, resolverPrevisoesPendentes, getLimiarRecomendado } from './services/calibrationService';
import HistoricoModal from './components/HistoricoModal';
import { getHistoricoStats } from './services/historicoService';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const cachedAnalyses: Record<string, AnalysisResponse> = {};
    const now = Date.now();
    const TTL = 4 * 60 * 60 * 1000; // 4 Hours (Gemini v8)

    keys.forEach(key => {
      if (key.startsWith('analysis_')) {
        const id = key.replace('analysis_', '');
        try {
          const { data, timestamp } = JSON.parse(localStorage.getItem(key) || '');
          if (now - timestamp < TTL) {
            cachedAnalyses[id] = data;
          }
        } catch (e) {
    ...
    if (Object.keys(cachedAnalyses).length > 0) {
      setAnalyzedMatches(cachedAnalyses);
    }
  }, []);
  ...
  const loadMatches = async (silent = false) => {
    ...
    setError(null);
    try {
      if (!ODDS_API_KEY || ODDS_API_KEY === 'MY_ODDS_API_KEY') {
        console.warn('Using mock data: API Key missing or default.');
      }
      const data = await fetchAllMatches(ODDS_API_KEY, selectedLeagues);
  ...
  const handleAnalyze = async (match: Match) => {
    ...
    try {
      ...
      const statsMedias = await buscarEstatisticasMedias(
        match.home_team,
        match.away_team,
        match.sport_title
      ).catch(() => null);
      ...
      // CORREÇÃO SCOUTING
      let forma = 50, motivacao = 50, desfalques = 50;
      try {
        const scoutData = await fetchRealScouting(match.home_team, match.away_team).catch(() => null);
        if (scoutData && scoutData.confiavel) {
          // forma: % de resultados positivos (V) nos últimos 5 jogos de ambos os times
          const allForm = [...(scoutData.home_form || []), ...(scoutData.away_form || [])];
          const known = allForm.filter(r => r !== '?');
          const wins = known.filter(r => r === 'V' || r === 'W').length;
          forma = known.length > 0 ? Math.round((wins / known.length) * 100) : 50;
          motivacao = scoutData.data_source === 'real' ? 70 : 55;
          desfalques = 75; // sem dados de lesões nesta fonte, assumir favorável
        }
      } catch (e) {
        forma = 50; motivacao = 50; desfalques = 50;
      }
      ...
      const engineInput = {
        ...
        // Passthrough to ensure we have all fields for the engine
        ...result
      };
      ...
      // Cache the complete result containing the engine verdict
      localStorage.setItem(`analysis_${match.id}`, JSON.stringify({ data: result, timestamp: Date.now() }));
    ...
    try {
      ...
      for (const match of toAnalyze) {
        try {
          ...
          const kellyPercentualValue = parseFloat(Math.min((kellyReaisValue / bancaAtualTotal) * 100, 5).toFixed(2));

          const confiancaDecimalValue = result.tipster?.confidence ?? (result.quality || 70) / 100 ?? 0;
  ...
  const handleAutoTicket = () => {
    const bestMatchIds = Object.entries(analyzedMatches)
      .filter(([id, analysisObj]) => {
        ...
        if (analysis.tipsterEngine && analysis.tipsterEngine.liberado === false) {
          return false;
        }
    ...
    if (bestMatchIds.length === 0) {
      alert('Nenhum jogo com confiança > 75% analisado ainda hoje. Analise mais jogos primeiro!');
      return;
    }
    ...
    <div translate="no" className="min-h-screen bg-[#0a0a0b] text-[#e1e1e3] font-sans selection:bg-blue-500/30">
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden xl:flex items-center gap-1.5 2xl:gap-4 shrink-0">
            ...
            <div className="flex items-center gap-1 2xl:gap-2 shrink-0">
              ...
              <button 
                onClick={() => setResultadoModalOpen(true)}
                className="h-9 flex items-center gap-1 px-2 2xl:gap-1.5 2xl:px-3.5 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/20 hover:border-amber-500/30 text-amber-400 rounded-xl font-mono text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer shadow-[0_0_15px_rgba(245,158,11,0.02)] active:scale-95 shrink-0 whitespace-nowrap"
              >
                📝 RESULTADO
              </button>
            </div>
            ...
            <div className="flex items-center gap-0.5 2xl:gap-1 bg-white/[0.02] border border-white/5 p-1 rounded-2xl shadow-inner shrink-0">
              ...
              >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest flex items-center gap-1 whitespace-nowrap">
                  Pendências
                  {(() => {
            ...
            <div className="relative shrink-0" ref={extraMenuRef}>
              <button
                ...
                className={`h-9 flex items-center justify-center px-3 bg-[#0f0f11]/40 hover:bg-white/10 border rounded-xl transition-all active:scale-95 cursor-pointer shrink-0 ${
                  isExtraMenuOpen || view === 'telemetry' || view === 'documentacao'
            ...
            <button
              ...
              className={`p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-all active:scale-95 shrink-0 ${
                isRefreshing ? 'animate-spin text-blue-500' : 'text-white/40 hover:text-white'
              }`}
            >
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 py-8 sm:py-10 overflow-x-hidden relative">
        ...
        {view === 'dashboard' ? (
          ...
          <WorldCupView onBack={() => setView('dashboard')} bancaAtual={bancaAtual} />
        ) : (
          <>
            {/* Intro & Summary */}
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 bg-[#0d0d0f] rounded-[2rem] border border-white/10 mt-10 shadow-2xl">
                ...
                <h3 className="text-2xl font-black text-white mb-3 uppercase tracking-tighter italic">
                  {searchQuery ? <span>Sem Resultados</span> : <span>Vazio</span>}
                </h3>
                      ...
                      {(leagueMatches as Match[]).map((match: Match) => (
                        <MatchCardTipster
                          ...
                          match={{
                            ...
                            resultado_registrado: match.resultado_registrado,
                            resultado_placar: match.resultado_placar,
